...
export async function GET(req: NextRequest) {
  try {
    const list = dbStore.getShipments();
    ...
    const decoratedList = list.map(s => {
      ...
      return {
        ...
        importer,
        exporter,
        assignments: shipmentAssignments
      };
    });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const newShipment: Shipment = {
      ...
      destinationPort,
      shipmentScope,
      status: 'ESCROW_FUNDED', // Set direct to funded once completed
      ...
      estimatedArrival: estimatedArrival || new Date(Date.now() + 10 * 24 * 60 * 60 * 100).toISOString(),
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };

    dbStore.saveShipment(newShipment);

    // Save assignments
    if (Array.isArray(selectedLogisticsUsers)) {
      selectedLogisticsUsers.forEach((lId: string) => {
        ...
        dbStore.saveAssignment(assignment);
      });
    }

    // Save Priority Milestones
    const milestonesList: PriorityMilestone[] = [];
    if (Array.isArray(requiredMilestones)) {
      requiredMilestones.forEach(mType => {
        milestonesList.push({
      ...
      // Defaults suggested: CUSTOMS_CLEARANCE_APPROVED, DELIVERED_AND_SIGNED_OFF
      ['CUSTOMS_CLEARANCE_APPROVED', 'DELIVERED_AND_SIGNED_OFF'].forEach(mType => {
        milestonesList.push({
          id: 'pm_' + Math.random().toString(36).substring(2, 9),
          shipmentId: newId,
          type: mType as any,
